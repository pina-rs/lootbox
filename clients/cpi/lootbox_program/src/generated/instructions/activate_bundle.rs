...
pub struct ActivateBundle<'account> {
	...
	/// Instruction arguments encoded and sent as CPI data for `activate_bundle`.
	pub ix: ActivateBundleIx,
}

/// Instruction arguments for the `activate_bundle` CPI call.
#[derive(Clone, Copy, Debug, Default)]
pub struct ActivateBundleIx;

impl ActivateBundleIx {
	...
	pub fn to_bytes(&self) -> Result<[u8; 2], ProgramError> {
		let mut data = [0u8; 2];
		data[..2].copy_from_slice(&ACTIVATE_BUNDLE_DISCRIMINATOR);

		Ok(data)
	}
}

impl<'account> ActivateBundle<'account> {
...
const ACTIVATE_BUNDLE_DISCRIMINATOR: [u8; 2] = [25, 0];
