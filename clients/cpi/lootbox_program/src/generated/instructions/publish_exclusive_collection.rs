...
pub struct PublishExclusiveCollection<'account> {
	...
	/// Instruction arguments encoded and sent as CPI data for `publish_exclusive_collection`.
	pub ix: PublishExclusiveCollectionIx,
}

/// Instruction arguments for the `publish_exclusive_collection` CPI call.
#[derive(Clone, Copy, Debug, Default)]
pub struct PublishExclusiveCollectionIx;

impl PublishExclusiveCollectionIx {
	...
	pub fn to_bytes(&self) -> Result<[u8; 2], ProgramError> {
		let mut data = [0u8; 2];
		data[..2].copy_from_slice(&PUBLISH_EXCLUSIVE_COLLECTION_DISCRIMINATOR);

		Ok(data)
	}
}

impl<'account> PublishExclusiveCollection<'account> {
...
const PUBLISH_EXCLUSIVE_COLLECTION_DISCRIMINATOR: [u8; 2] = [56, 0];
