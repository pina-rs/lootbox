...
Unit/property tests cover finite inventory conservation, exhausted odds, pending liability, FIFO rejection, per-asset claim replay, destination binding, duplicate collateral, overflow, metadata bounds, and retirement liabilities. The Surfpool journey exercises real SBF and real token programs, with only the oracle emulated. The v2 UI, browser-to-RPC workflow, hosted deployment, and an independent audit are not complete yet; consult the live checklist in the v2 specification.
