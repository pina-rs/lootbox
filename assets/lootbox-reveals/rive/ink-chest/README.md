...
Use Node 24 or later and Rive CLI 1.0.4. No Rive login or script signing is required.
...
The build replaces only the marked RML sections, compiles `ink-chest.riv`, and renders four PNG stills into `apps/web/public/animations/ink-chest`. Commit the source and these outputs together. Website builds use the checked-in outputs without requiring the Rive CLI.
...
The browser tests exercise the native Rive file, all three reactions, replay, interruption, skipping, reduced motion, download failure, and the real prize claim flow. Claiming never waits for the illustration.
...
The artboard is 640 × 640 with an opaque warm-paper background. Each reaction lasts five seconds. The big celebration uses 48 secondary stars. The website self-hosts the Rive file and WASM, loads the runtime on demand, and shows a matching PNG for reduced motion or failed playback.
