...
The treasury protocol includes compact append-only treasuries with up to 1,024 bundles, fully funded SOL/token/NFT bundles, entropy-selected Bubblegum PrizePools, layered Exclusive Lootbox NFTs minted on claim, winner-routed quote prizes, capped badge minting, irreversible exact-supply locking, immutable Token-2022 box metadata, reveal dates, FIFO allocation, independent claims, staged-funding cancellation, timeout forfeiture, safe retirement, and a bounded non-market recovery seal for a missed lock deadline. See the [treasury specification](docs/treasury-templates.md), [PrizePool reference](docs/prize-pools.md), [Exclusive Lootbox NFTs](docs/exclusive-nfts.md), [dynamic prize design](docs/dynamic-prizes.md), and [treasury security notes](docs/security-templates.md).
...
- Fully escrowed finite SOL, classic SPL, safe Token-2022, standard Token Metadata NFT, Core, compressed-NFT, PrizePool, SOL/token quote, and badge-mint bundles in treasury templates; the single-reward SOL model retains minimum-reward timeouts.
...
tests/fixtures/                 test-only Switchboard and Bubblegum SBF programs
