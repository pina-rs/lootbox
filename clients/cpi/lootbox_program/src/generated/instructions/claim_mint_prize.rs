...
/// CPI call for the `claim_mint_prize` instruction.
#[derive(Clone, Copy, Debug)]
#[must_use = "the CPI has no effect until invoke or invoke_signed is called"]
pub struct ClaimMintPrize<'account> {
	...
	/// Instruction arguments encoded and sent as CPI data for `claim_mint_prize`.
	pub ix: ClaimMintPrizeIx,
}

/// Instruction arguments for the `claim_mint_prize` CPI call.
#[derive(Clone, Copy, Debug)]
...
impl ClaimMintPrizeIx {
	...
	pub fn to_bytes(&self) -> Result<[u8; 3], ProgramError> {
		let mut data = [0u8; 3];
		data[..2].copy_from_slice(&CLAIM_MINT_PRIZE_DISCRIMINATOR);
...
impl<'account> ClaimMintPrize<'account> {
...
const CLAIM_MINT_PRIZE_DISCRIMINATOR: [u8; 2] = [42, 0];
