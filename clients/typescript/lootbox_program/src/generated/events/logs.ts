 ...
 */
const EXCLUSIVE_NFT_MINTED_EVENT_EVENT_PROJECTION_STEPS: readonly ExclusiveNftMintedEventEventProjectionStep[] = [

];

/** Versions whose adjacent transition is manual and not derivable in clients. */
const EXCLUSIVE_NFT_MINTED_EVENT_EVENT_MANUAL_VERSIONS: readonly number[] = [];

const EXCLUSIVE_NFT_MINTED_EVENT_EVENT_HEADER_SIZE = 2;
const EXCLUSIVE_NFT_MINTED_EVENT_EVENT_CURRENT_VERSION = 0;

/** Raw event bytes with their source version and whether a projection ran. */
export type NormalizedExclusiveNftMintedEventEvent = {
	name: "exclusiveNftMintedEvent";
	data: ExclusiveNftMintedEventEvent;
 ...
 */
export function normalizeExclusiveNftMintedEventEvent(
	data: ReadonlyUint8Array | Uint8Array,
): NormalizedExclusiveNftMintedEventEvent {
	const bytes = data instanceof Uint8Array ? data : new Uint8Array(data);
	const discriminatorBytes = getExclusiveNftMintedEventEventDiscriminatorBytes();
	if (bytes.length < EXCLUSIVE_NFT_MINTED_EVENT_EVENT_HEADER_SIZE) {
		throw new RangeError(
			`the provided data is too short for the "ExclusiveNftMintedEventEvent" event envelope`,
		);
	}
	for (let index = 0; index < 1; index += 1) {
		if (bytes[index] !== discriminatorBytes[index]) {
			throw new RangeError(
				'the provided data does not match the "ExclusiveNftMintedEventEvent" event discriminator.',
			);
		}
	}
	...
	if (sourceVersion === EXCLUSIVE_NFT_MINTED_EVENT_EVENT_CURRENT_VERSION) {
		return {
			name: "exclusiveNftMintedEvent",
			data: getExclusiveNftMintedEventEventDecoder().decode(bytes),
			sourceVersion,
			wasMigrated: false,
		};
	}
	...
	const projected = projectExclusiveNftMintedEventEvent(bytes, sourceVersion);
	return {
		name: "exclusiveNftMintedEvent",
		data: getExclusiveNftMintedEventEventDecoder().decode(projected),
		sourceVersion,
		wasMigrated: true,
	};
}
...
export type DecodedExclusiveNftMintedEventEvent = NormalizedExclusiveNftMintedEventEvent;
 ...
 */
export function parseExclusiveNftMintedEventEventFromLog(log: string): DecodedExclusiveNftMintedEventEvent | null {
...
function projectExclusiveNftMintedEventEvent(
	bytes: Uint8Array,
	sourceVersion: number,
): Uint8Array {
	const discriminatorBytes = getExclusiveNftMintedEventEventDiscriminatorBytes();
	let version = sourceVersion;
	let payload = bytes.slice(EXCLUSIVE_NFT_MINTED_EVENT_EVENT_HEADER_SIZE);
	while (version !== EXCLUSIVE_NFT_MINTED_EVENT_EVENT_CURRENT_VERSION) {
		const step = EXCLUSIVE_NFT_MINTED_EVENT_EVENT_PROJECTION_STEPS.find(
			(candidate) => candidate.from === version,
		);
		if (step === undefined || !step.automatic) {
			const reason = EXCLUSIVE_NFT_MINTED_EVENT_EVENT_MANUAL_VERSIONS.includes(version)
	...
	const projected = new Uint8Array(EXCLUSIVE_NFT_MINTED_EVENT_EVENT_HEADER_SIZE + payload.length);
	projected.set(discriminatorBytes, 0);
	const header = 1;
	projected[header] = 0;
	projected.set(payload, EXCLUSIVE_NFT_MINTED_EVENT_EVENT_HEADER_SIZE);
	return projected;
}
 ...
 */
export function parseLootboxProgramEventsFromLogs(
	logs: readonly string[],
): DecodedLootboxProgramEvent[] {
	const discovered: DecodedLootboxProgramEvent[] = [];
