...
pub struct Seal<'account> {
	...
	/// Instruction arguments encoded and sent as CPI data for `seal`.
	pub ix: SealIx,
}

/// Instruction arguments for the `seal` CPI call.
...
impl SealIx {
	...
	pub fn to_bytes(&self) -> Result<[u8; 2], ProgramError> {
		let mut data = [0u8; 2];
		data[..2].copy_from_slice(&SEAL_DISCRIMINATOR);

		Ok(data)
	}
}

impl<'account> Seal<'account> {
...
const SEAL_DISCRIMINATOR: [u8; 2] = [3, 0];
