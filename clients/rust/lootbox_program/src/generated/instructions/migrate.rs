//! This code was AUTOGENERATED by `pina_codama_renderer`.
//! Please DO NOT EDIT THIS FILE directly.
//! Regenerate it from Codama IDLs instead.
#![allow(
	clippy::doc_lazy_continuation,
	clippy::empty_line_after_doc_comments,
	clippy::new_without_default,
	clippy::too_many_arguments
)]

/// Discriminator reserved by Pina for the framework `Migrate` instruction.
pub const MIGRATE_DISCRIMINATOR: u8 = 255u8;

/// The reserved discriminator bytes carried as the instruction payload.
pub fn get_migrate_discriminator_bytes() -> Vec<u8> {
	vec![255]
}

/// Accounts.
#[derive(Clone, Debug)]
pub struct Migrate {
	pub payer: Option<solana_pubkey::Pubkey>,
	pub system_program: Option<solana_pubkey::Pubkey>,
	pub lootbox_state: Option<solana_pubkey::Pubkey>,
	pub vault_state: Option<solana_pubkey::Pubkey>,
	pub opening_state: Option<solana_pubkey::Pubkey>,
	pub template_state: Option<solana_pubkey::Pubkey>,
	pub bundle_state: Option<solana_pubkey::Pubkey>,
	pub template_opening_state: Option<solana_pubkey::Pubkey>,
	pub result_receipt_state: Option<solana_pubkey::Pubkey>,
	pub prize_pool_state: Option<solana_pubkey::Pubkey>,
	pub prize_pool_item_state: Option<solana_pubkey::Pubkey>,
	pub exclusive_series_state: Option<solana_pubkey::Pubkey>,
}

impl Migrate {
	pub fn new() -> Self {
		Self {
			payer: None,
			system_program: None,
			lootbox_state: None,
			vault_state: None,
			opening_state: None,
			template_state: None,
			bundle_state: None,
			template_opening_state: None,
			result_receipt_state: None,
			prize_pool_state: None,
			prize_pool_item_state: None,
			exclusive_series_state: None,
		}
	}

	/// Composes the reserved `Migrate` instruction.
	///
	/// Omitted accounts become program-address placeholders, which the
	/// program treats as absent: send only the accounts that are stale.
	/// The payer must sign and be writable when any migration needs funding.
	pub fn instruction(&self) -> solana_instruction::Instruction {
		self.instruction_with_remaining_accounts(&[])
	}

	pub fn instruction_with_remaining_accounts(
		&self,
		remaining_accounts: &[solana_instruction::AccountMeta],
	) -> solana_instruction::Instruction {
		let mut accounts = Vec::with_capacity(12 + remaining_accounts.len());
		if let Some(payer) = self.payer {
			accounts.push(solana_instruction::AccountMeta::new(payer, true));
		} else {
			accounts.push(solana_instruction::AccountMeta::new_readonly(
				crate::LOOTBOX_PROGRAM_ID,
				false,
			));
		}
		if let Some(system_program) = self.system_program {
			accounts.push(solana_instruction::AccountMeta::new_readonly(
				system_program,
				false,
			));
		} else {
			accounts.push(solana_instruction::AccountMeta::new_readonly(
				crate::LOOTBOX_PROGRAM_ID,
				false,
			));
		}
		if let Some(lootbox_state) = self.lootbox_state {
			accounts.push(solana_instruction::AccountMeta::new(lootbox_state, false));
		} else {
			accounts.push(solana_instruction::AccountMeta::new_readonly(
				crate::LOOTBOX_PROGRAM_ID,
				false,
			));
		}
		if let Some(vault_state) = self.vault_state {
			accounts.push(solana_instruction::AccountMeta::new(vault_state, false));
		} else {
			accounts.push(solana_instruction::AccountMeta::new_readonly(
				crate::LOOTBOX_PROGRAM_ID,
				false,
			));
		}
		if let Some(opening_state) = self.opening_state {
			accounts.push(solana_instruction::AccountMeta::new(opening_state, false));
		} else {
			accounts.push(solana_instruction::AccountMeta::new_readonly(
				crate::LOOTBOX_PROGRAM_ID,
				false,
			));
		}
		if let Some(template_state) = self.template_state {
			accounts.push(solana_instruction::AccountMeta::new(template_state, false));
		} else {
			accounts.push(solana_instruction::AccountMeta::new_readonly(
				crate::LOOTBOX_PROGRAM_ID,
				false,
			));
		}
		if let Some(bundle_state) = self.bundle_state {
			accounts.push(solana_instruction::AccountMeta::new(bundle_state, false));
		} else {
			accounts.push(solana_instruction::AccountMeta::new_readonly(
				crate::LOOTBOX_PROGRAM_ID,
				false,
			));
		}
		if let Some(template_opening_state) = self.template_opening_state {
			accounts.push(solana_instruction::AccountMeta::new(template_opening_state, false));
		} else {
			accounts.push(solana_instruction::AccountMeta::new_readonly(
				crate::LOOTBOX_PROGRAM_ID,
				false,
			));
		}
		if let Some(result_receipt_state) = self.result_receipt_state {
			accounts.push(solana_instruction::AccountMeta::new(result_receipt_state, false));
		} else {
			accounts.push(solana_instruction::AccountMeta::new_readonly(
				crate::LOOTBOX_PROGRAM_ID,
				false,
			));
		}
		if let Some(prize_pool_state) = self.prize_pool_state {
			accounts.push(solana_instruction::AccountMeta::new(prize_pool_state, false));
		} else {
			accounts.push(solana_instruction::AccountMeta::new_readonly(
				crate::LOOTBOX_PROGRAM_ID,
				false,
			));
		}
		if let Some(prize_pool_item_state) = self.prize_pool_item_state {
			accounts.push(solana_instruction::AccountMeta::new(prize_pool_item_state, false));
		} else {
			accounts.push(solana_instruction::AccountMeta::new_readonly(
				crate::LOOTBOX_PROGRAM_ID,
				false,
			));
		}
		if let Some(exclusive_series_state) = self.exclusive_series_state {
			accounts.push(solana_instruction::AccountMeta::new(exclusive_series_state, false));
		} else {
			accounts.push(solana_instruction::AccountMeta::new_readonly(
				crate::LOOTBOX_PROGRAM_ID,
				false,
			));
		}
		accounts.extend_from_slice(remaining_accounts);
		solana_instruction::Instruction {
			program_id: crate::LOOTBOX_PROGRAM_ID,
			accounts,
			data: get_migrate_discriminator_bytes(),
		}
	}
}
