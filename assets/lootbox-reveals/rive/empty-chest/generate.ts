...
function shape(
	...
	const fillPaint = fill ? `<Fill><SolidColor colorValue="${fill}"/></Fill>` : "";
...
type Glyph = Readonly<{ width: number; strokes: readonly (readonly Point[])[] }>;
...
const glyphs: Readonly<Record<string, Glyph>> = {
	A: { width: .62, strokes: [[[0, 1], [.31, 0], [.62, 1]], [[.13, .64], [.49, .64]]] },
	D: { width: .6, strokes: [[[0, 0], [0, 1], [.32, 1], [.6, .72], [.6, .28], [.32, 0], [0, 0]]] },
	E: { width: .52, strokes: [[[.52, 0], [0, 0], [0, 1], [.52, 1]], [[0, .5], [.4, .5]]] },
	H: { width: .6, strokes: [[[0, 0], [0, 1]], [[.6, 0], [.6, 1]], [[0, .5], [.6, .5]]] },
	I: { width: .44, strokes: [[[.22, 0], [.22, 1]], [[0, 0], [.44, 0]], [[0, 1], [.44, 1]]] },
	L: { width: .5, strokes: [[[0, 0], [0, 1], [.5, 1]]] },
	O: { width: .62, strokes: [oval] },
	R: {
		width: .6,
		strokes: [[[0, 1], [0, 0], [.42, 0], [.6, .16], [.6, .34], [.42, .5], [0, .5]], [[.28, .5], [.6, 1]]],
	},
	S: {
		width: .58,
		strokes: [[[.56, .12], [.42, 0], [.14, 0], [0, .14], [0, .34], [.14, .47], [.44, .53], [.58, .66], [.58, .86], [.44, 1], [.14, 1], [0, .88]]],
	},
	T: { width: .6, strokes: [[[0, 0], [.6, 0]], [[.3, 0], [.3, 1]]] },
	U: { width: .6, strokes: [[[0, 0], [0, .76], [.2, 1], [.4, 1], [.6, .76], [.6, 0]]] },
	"0": { width: .54, strokes: [oval.map(([x, y]) => [x * .87, y] as const), [[.12, .8], [.42, .2]]] },
	"?": { width: .52, strokes: [[[0, .2], [.12, .03], [.38, 0], [.52, .15], [.52, .32], [.26, .5], [.26, .68]], [[.26, .92], [.26, .95]]] },
...
function place(tracks: readonly Track[], start: number, duration: number) {
	return tracks.map((track): Track => {
		...
		if (!first || !last || first[0] !== 0 || last[0] !== ACTION_FRAMES) {
			throw new Error(`Action track 0:${track.id} must span 0..${ACTION_FRAMES}`);
		}
...
function thing(name: string, root: number, size: number, children: readonly string[]) {
...
function moth(root: number): Variant {
	const wing = (side: 1 | -1) =>
		shape(
			...
			poly([[0, 0], [30 * side, -26], [48 * side, -14], [42 * side, 8], [18 * side, 16]], true, 9),
			"FFE8DAB6",
		) +
	...
	return {
		...
		action: [
			track(root, Y, [[0, 0, "out"], [6, -150], [12, -240], [18, -310], [24, -340], [30, -300], [36, -250], [42, -190, "in"], [47, -60], [ACTION_FRAMES, 0]]),
			track(root, X, [[0, 0], [12, 50], [18, 120], [24, 70], [30, -60], [36, -115], [42, -40], [47, 0], [ACTION_FRAMES, 0]]),
			track(root, ROTATION, [[0, 0], [12, .35], [18, .1], [24, -.4], [30, -.5], [36, .2], [42, .3], [ACTION_FRAMES, 0]]),
			flutter(root + 1, 2, 46),
			flutter(root + 2, 2, 46),
		],
	};
	...
	return {
		art: thing("Odd sock", root, 1.25, [
			group("Floppy foot", root + 1, { y: -70 }, [
				shape("Toe patch", poly([[30, -62], [44, -62], [56, -50], [52, -34], [36, -30]], true, 6), gold, 3),
				shape("Heel patch", poly([[-21, -30], [-21, -50], [-8, -62], [2, -58], [-6, -32]], true, 5), gold, 3),
				line("Foot stripe", [[-19, -16], [19, -16]], teal, 6),
				shape("Sock foot", poly([[-21, 4], [-21, -48], [-6, -63], [42, -63], [56, -50], [52, -32], [21, -28], [21, 4]], true, 10), coral),
			]),
			...stripes,
			shape("Sock cuff ribs", poly([[-10, -4], [-10, -12]], false), undefined, 2),
			shape("Sock leg", rect(0, -36, 42, 74, 4), coral),
		]),
		action: [
			track(root, Y, [[0, 0, "out"], [8, -222], [12, -196], [16, -204], [40, -200], [46, -50, "in"], [ACTION_FRAMES, 0]]),
			track(root, ROTATION, [[0, 0], [10, -.14], [20, .1], [30, -.06], [40, .05], [ACTION_FRAMES, 0]]),
			track(root + 1, ROTATION, [[0, 0], [8, -.25], [14, .95], [20, .45], [26, .9], [32, .55], [38, .8], [44, .15], [ACTION_FRAMES, 0]]),
		],
	};
}
...
function iou(root: number): Variant {
	return {
		art: thing("IOU", root, 1.45, [
			line("Signature", [[18, -18], [26, -26], [30, -16], [38, -26], [46, -18], [58, -20]], ink, 2),
		...
		action: [
			track(root, Y, [[0, 0, "out"], [8, -215], [12, -196], [16, -202], [40, -200], [46, -60, "in"], [ACTION_FRAMES, 0]]),
			track(root, ROTATION, [[0, 0], [8, -.16], [14, .08], [20, -.05], [28, .03], [40, 0], [ACTION_FRAMES, 0]]),
			track(root, SCALE_X, [[0, 1], [10, 1], [14, 1.1], [18, 1], [ACTION_FRAMES, 1]]),
			track(root, SCALE_Y, [[0, 1], [10, 1], [14, .9], [18, 1], [ACTION_FRAMES, 1]]),
		],
	};
}
...
function cobweb(root: number): Variant {
	...
	const ring = (radius: number) =>
		poly(
			Array.from({ length: 8 }, (_, i) => {
				const angle = i * Math.PI / 4 + .2;
				const sag = i % 2 ? .86 : 1;
				return [Math.cos(angle) * radius * sag, Math.sin(angle) * radius * sag - 70] as const;
			}),
			true,
		);
	const legs = [-1, 1].flatMap((side) =>
		[-6, 0, 6].map((y) =>
			line("Spider leg", [[4 * side, y], [13 * side, y - 7], [18 * side, y + 4]], ink, 2.2)
		)
	);
	...
	return {
		art: thing("Cobweb", root, 1.3, [
			...
			shape("Web", spokes.map((point) => poly([[0, -70], point], false)).join("") + ring(20) + ring(38) + ring(56), undefined, 2.4, "FF9FB2AC"),
		]),
		action: [
			track(root, Y, [[0, 0, "out"], [10, -250], [14, -236], [40, -236], [46, -60, "in"], [ACTION_FRAMES, 0]]),
			track(root + 1, Y, [[0, -70], [14, -70, "in"], [20, 0], [24, -14], [28, 8], [32, -6], [36, 0], [41, -70, "in"], [ACTION_FRAMES, -70]]),
			track(root + 2, SCALE_Y, [[0, 1], [14, 1, "in"], [20, 6], [24, 5], [28, 6.6], [32, 5.6], [36, 6], [41, 1, "in"], [ACTION_FRAMES, 1]]),
			track(root + 3, ROTATION, [[0, 0], [20, 0], [24, .35], [28, -.3], [32, .2], [36, 0], [ACTION_FRAMES, 0]]),
		],
	};
}
...
function dustBunny(root: number): Variant {
	return {
		art: thing("Dust bunny", root, 1.4, [
			...
			shape("Blush", ellipse(26, -34, 10, 5), "FFF2A89C", 0),
			shape("Fluff", `<Star x="0" y="-40" width="96" height="82" points="22" innerRadius="0.84"/>`, "FFBDB5A4"),
			shape("Ear", poly([[-18, -70], [-30, -112], [-16, -116], [-8, -76]], true, 7), "FFBDB5A4"),
			shape("Ear", poly([[10, -76], [22, -118], [36, -110], [22, -70]], true, 7), "FFBDB5A4"),
		]),
		action: [
			track(root, Y, [[0, 0, "out"], [8, -230], [12, -350, "in"], [16, LID_TOP], [38, LID_TOP, "out"], [42, -370, "in"], [47, -80], [ACTION_FRAMES, 0]]),
			track(root, X, [[0, 0], [12, 30], [16, 45], [38, 45], [42, 25], [ACTION_FRAMES, 0]]),
			track(root, SCALE_Y, [[0, 1], [15, 1.1], [17, .72], [21, 1.08], [25, 1], [28, 1], [30, .85], [32, 1.25], [36, 1], [38, 1], [40, .8], [42, 1.15], [ACTION_FRAMES, 1]]),
			track(root, SCALE_X, [[0, 1], [15, .92], [17, 1.24], [21, .96], [25, 1], [28, 1], [30, 1.1], [32, .9], [36, 1], [38, 1], [40, 1.2], [42, .9], [ACTION_FRAMES, 1]]),
			track(root + 1, SCALE_Y, [[0, 1], [22, 1, "linear"], [23, .1, "linear"], [25, 1, "linear"], [30, 1, "linear"], [31, .1, "linear"], [35, .1, "linear"], [36, 1], [ACTION_FRAMES, 1]]),
		],
	};
}
...
function duck(root: number): Variant {
	return {
		art: thing("Rubber duck", root, 1.5, [
			...
			shape("Beak", poly([[42, -76], [66, -74], [64, -62], [42, -60]], true, 5), "FFF08A3C"),
			shape("Wing", poly([[-22, -44], [-4, -50], [10, -40], [-8, -30]], true, 8), "FFF2BF2E", 3),
			shape("Head", ellipse(24, -72, 50, 48), "FFF7D046"),
			shape("Belly shine", poly([[-30, -20], [-10, -14], [10, -14]], false), undefined, 3, "FFFFF1BA"),
			shape("Duck body", poly([[-54, -54], [-40, -46], [-4, -48], [24, -50], [52, -40], [48, -10], [22, 0], [-30, 0], [-50, -16]], true, 16), "FFF7D046"),
		]),
		action: [
			track(root, Y, [[0, 0, "out"], [8, -236], [12, -206], [16, -218], [20, -208], [24, -214], [40, -210], [46, -60, "in"], [ACTION_FRAMES, 0]]),
			track(root, ROTATION, [[0, 0], [12, .12], [18, -.08], [26, .06], [34, -.04], [40, 0], [ACTION_FRAMES, 0]]),
			track(root, SCALE_Y, [[0, 1], [24, 1, "linear"], [26, .8, "out"], [29, 1.1], [33, 1], [ACTION_FRAMES, 1]]),
			track(root, SCALE_X, [[0, 1], [24, 1, "linear"], [26, 1.14, "out"], [29, .95], [33, 1], [ACTION_FRAMES, 1]]),
			track(root + 1, OPACITY, [[0, 0], [25, 0, "hold"], [26, 1], [34, 1, "linear"], [37, 0], [ACTION_FRAMES, 0]]),
			track(root + 1, SCALE_X, [[0, .5], [25, .5, "out"], [29, 1.2], [34, 1], [ACTION_FRAMES, .5]]),
			track(root + 1, SCALE_Y, [[0, .5], [25, .5, "out"], [29, 1.2], [34, 1], [ACTION_FRAMES, .5]]),
		],
	};
}
	...
	return {
		art: thing("Sold out tag", root, 1.2, [
			group("Tag swing", root + 1, { y: -140 }, [
				...
				shape("Tag", poly([[-44, 135], [44, 135], [44, 60], [22, 38], [-22, 38], [-44, 60]], true, 7), "FFEBCB8E"),
				line("String", [[0, 50], [-12, 26], [-4, 0], [10, 20], [0, 50]], ink, 2.2),
			]),
		]),
		action: [
			track(root, Y, [[0, 0, "out"], [8, -262], [12, -240], [16, -246], [40, -244], [46, -60, "in"], [ACTION_FRAMES, 0]]),
			track(root + 1, ROTATION, [[0, 0], [10, .5], [16, -.38], [22, .26], [28, -.16], [34, .08], [40, 0], [ACTION_FRAMES, 0]]),
	...
	return {
		art: thing("Lost button", root, 1.5, [
			group("Rolling button", root + 1, { y: -36 }, [
				...
				shape("Button shine", poly([[-24, -12], [-18, -22], [-8, -28]], false), undefined, 3, "FFFFC2B4"),
				shape("Button", ellipse(0, 0, 72, 72), coral),
			]),
		]),
		action: [
			track(root, Y, [[0, 0, "out"], [8, -340], [11, LID_TOP, "out"], [14, -322], [17, LID_TOP], [42, LID_TOP, "out"], [44, -340, "in"], [49, -40], [ACTION_FRAMES, 0]]),
			track(root, X, [[0, 0], [17, 0], [27, 100], [37, -90], [42, -20], [49, 0], [ACTION_FRAMES, 0]]),
			track(root + 1, ROTATION, [[0, 0], [17, 0], [27, roll(100)], [37, roll(-90)], [42, roll(-20)], [49, 0], [ACTION_FRAMES, 0]]),
		],
	};
}
	...
	return {
		art: thing("Paper crown", root, 1.5, [
			...[[-20, coral], [0, teal], [20, coral]].map(([x, color]) =>
				shape("Paper jewel", ellipse(Number(x), -14, 11, 11), String(color), 2.5)
			),
			...tips.map(([x, y]) => shape("Crown tip", ellipse(x, y, 10, 10), gold, 2.5)),
			line("Crease", [[-30, -30], [-26, -6]], "FFD9A635", 2),
			line("Crease", [[28, -34], [24, -8]], "FFD9A635", 2),
			line("Band", [[-50, -24], [50, -24]], "FFD9A635", 2.5),
			shape("Crown", poly([[-50, 0], [-50, -44], [-34, -24], [-18, -56], [0, -28], [18, -56], [34, -24], [50, -44], [50, 0]], true, 3), gold),
		]),
		action: [
			track(root, Y, [[0, 0, "out"], [8, -360], [13, LID_TOP + 6, "out"], [16, LID_TOP], [40, LID_TOP, "out"], [44, -350, "in"], [49, -60], [ACTION_FRAMES, 0]]),
			track(root, X, [[0, 0], [13, -46], [40, -46], [44, -20], [ACTION_FRAMES, 0]]),
			track(root, ROTATION, [[0, 0], [8, -.5], [13, .22], [16, .14], [40, .14], [44, -.25], [ACTION_FRAMES, 0]]),
			track(root, SCALE_X, [[0, 1], [22, 1], [25, 1.12], [28, 1], [ACTION_FRAMES, 1]]),
			track(root, SCALE_Y, [[0, 1], [22, 1], [25, 1.12], [28, 1], [ACTION_FRAMES, 1]]),
		],
	};
}

function snail(root: number): Variant {
	const spiral: readonly Point[] = [[4, -44], [12, -46], [16, -38], [10, -30], [-2, -32], [-8, -44], [0, -58], [16, -60], [28, -48], [26, -30]];
	...
	return {
		art: thing("Snail", root, 1.5, [
			...
			shape("Snail body", poly([[-54, 0], [-50, -10], [24, -14], [34, -34], [52, -38], [64, -24], [62, 0]], true, 9), "FFB3CF92"),
		]),
		action: [
			track(root, Y, [[0, 0], [16, -210], [38, -214], [48, -30], [ACTION_FRAMES, 0]]),
			track(root, X, [[0, 0], [16, -10], [38, 18], [ACTION_FRAMES, 0]]),
			track(root + 1, SCALE_Y, [[0, .2], [16, .2, "out"], [20, 1.12], [23, 1], [34, 1], [40, .3], [ACTION_FRAMES, .2]]),
			track(root + 1, ROTATION, [[0, 0], [22, 0], [26, .3], [30, -.28], [34, 0], [ACTION_FRAMES, 0]]),
		],
	};
}
	...
	return {
		art: thing("Crumpled receipt", root, 1.25, [
			group("Paper ball", root + 2, { y: -26, opacity: 0 }, [
				...
				shape("Crumpled ball", poly([[-22, -6], [-14, -22], [4, -24], [20, -14], [24, 4], [12, 20], [-8, 22], [-22, 10]], true, 5), paper),
			]),
		...
		action: [
			track(root, Y, [[0, 0, "out"], [8, -196], [34, -196, "out"], [40, -245, "in"], [47, -50], [ACTION_FRAMES, 0]]),
			track(root, ROTATION, [[0, 0], [16, 0], [20, .08], [26, -.06], [32, 0], [35, 0], [46, 3.2, "hold"], [ACTION_FRAMES, 0]]),
			track(root + 1, SCALE_Y, [[0, .3], [6, .3, "out"], [16, 1.06], [20, 1], [30, 1, "in"], [34, .2, "hold"], [49, .2], [ACTION_FRAMES, .3]]),
			track(root + 1, OPACITY, [[0, 1, "hold"], [34, 0, "hold"], [49, 0, "hold"], [ACTION_FRAMES, 1]]),
			track(root + 2, OPACITY, [[0, 0, "hold"], [34, 1, "hold"], [49, 1, "hold"], [ACTION_FRAMES, 0]]),
		],
	};
}
	...
	return {
		art: thing("Ghost certificate", root, 1.45, [
			group("Ghost body", root + 1, { opacity: .86 }, [
				...
				shape("Ghost eye", ellipse(14, -62, 9, 14), ink, 0),
				shape("Seal", `<Star x="30" y="-24" width="22" height="22" points="8" innerRadius="0.7"/>`, gold, 2),
				lettering("Share", "SHARE", 0, -100, 10, teal, 2),
				shape("Certificate border", rect(0, -60, 80, 94, 4), undefined, 2, "FFD9B45A"),
				shape("Certificate", poly([...hem.reverse(), [-48, -112], [48, -112]], true, 6), "FFF7F1DE"),
			]),
		]),
		action: [
			track(root, Y, [[0, 0, "out"], [10, -250], [16, -232], [22, -252], [28, -234], [34, -250], [40, -238], [47, -60, "in"], [ACTION_FRAMES, 0]]),
			track(root, X, [[0, 0], [10, -22], [16, 18], [22, -14], [28, 18], [34, -10], [40, 0], [ACTION_FRAMES, 0]]),
			track(root, ROTATION, [[0, 0], [10, -.1], [16, .1], [22, -.08], [28, .08], [34, -.05], [40, 0], [ACTION_FRAMES, 0]]),
			track(root, SCALE_X, [[0, 1], [26, 1], [28, 1.22, "out"], [31, .94], [34, 1], [ACTION_FRAMES, 1]]),
			track(root, SCALE_Y, [[0, 1], [26, 1], [28, 1.22, "out"], [31, .94], [34, 1], [ACTION_FRAMES, 1]]),
		],
	};
}
...
function echo(root: number): Variant {
	const bubble = (name: string) => [
		...
		shape(`${name} tail`, poly([[-30, -20], [-40, 2], [-12, -20]], true, 3), paper),
		shape(name, rect(0, -44, 148, 52, 22), paper),
	];

	return {
		art: thing("Echo", root, 1.35, [
			group("Faint echo", root + 3, { x: 70, y: -170, scaleX: .5, scaleY: .5, opacity: 0 }, bubble("Faint bubble")),
			group("Echo", root + 2, { x: 44, y: -96, scaleX: .72, scaleY: .72, opacity: 0 }, bubble("Echo bubble")),
			group("Hello", root + 1, {}, bubble("Speech bubble")),
		]),
		action: [
			track(root, Y, [[0, 0, "out"], [8, -226], [12, -212], [40, -214], [46, -60, "in"], [ACTION_FRAMES, 0]]),
			track(root + 1, ROTATION, [[0, 0], [12, -.05], [18, .03], [24, 0], [ACTION_FRAMES, 0]]),
			track(root + 2, OPACITY, [[0, 0], [16, 0], [19, .7], [30, .45], [36, 0], [ACTION_FRAMES, 0]]),
			track(root + 2, Y, [[0, -96], [16, -86], [36, -120], [ACTION_FRAMES, -96]]),
			track(root + 3, OPACITY, [[0, 0], [23, 0], [26, .42], [34, .25], [39, 0], [ACTION_FRAMES, 0]]),
			track(root + 3, Y, [[0, -170], [23, -160], [39, -200], [ACTION_FRAMES, -170]]),
		],
	};
}
...
section(
	...
	[
		...Array.from({ length: MOTES }, (_, i) =>
			group("Dust mote", moteId(i), { x: -40 + i * 26, y: -190, opacity: 0 }, [
				shape("Mote", ellipse(0, 0, 5 + (i % 2) * 3, 5 + (i % 2) * 3), "FFD8CCAA", 0),
			])
		),
		...Array.from({ length: PUFFS }, (_, i) =>
			group("Dust puff", puffId(i), { x: i < 3 ? -150 : 150, y: -184, scaleX: 0, scaleY: 0 }, [
				shape("Puff", ellipse(0, 0, 30 - (i % 3) * 5, 22 - (i % 3) * 4), dust, 2, "FFB8AC8A"),
			])
		),
	].join("\n"),
);
...
function dustBurst(start: number, duration: number): Track[] {
	...
	for (let i = 0; i < PUFFS; i++) {
		...
		const scale: Key[] = [[0, 0, "hold"], [at, 0, "out"], [at + 8, 1.1 - tier * .15], [at + 22, 1.4 - tier * .2, "hold"], [at + 23, 0, "hold"], [duration, 0, "hold"]];

		tracks.push(
			track(puffId(i), X, [[0, x, "hold"], [at, x, "out"], [at + 22, endX, "hold"], [duration, x, "hold"]]),
			track(puffId(i), Y, [[0, -184, "hold"], [at, -184, "out"], [at + 22, endY, "hold"], [duration, -184, "hold"]]),
			track(puffId(i), SCALE_X, scale),
			track(puffId(i), SCALE_Y, scale),
			track(puffId(i), OPACITY, [[0, 1, "hold"], [at + 6, 1, "in"], [at + 22, 0, "hold"], [duration, 1, "hold"]]),
		);
	}
...
function motes(start: number, duration: number): Track[] {
	return Array.from({ length: MOTES }, (_, i) => {
		...
		return [
			track(moteId(i), Y, [[0, -190, "hold"], [at, -190, "linear"], [at + 40, -300 - i * 14, "hold"], [duration, -190, "hold"]]),
			track(moteId(i), X, [[0, x, "hold"], [at, x], [at + 20, x + 14], [at + 40, x - 6, "hold"], [duration, x, "hold"]]),
			track(moteId(i), OPACITY, [[0, 0, "hold"], [at, 0, "linear"], [at + 12, .9, "linear"], [at + 40, 0, "hold"], [duration, 0, "hold"]]),
		];
	}).flat();
}

type LidPose = Readonly<{ frames: readonly number[]; lifts: readonly number[]; tilts: readonly number[] }>;

function lidTracks(pose: LidPose): Track[] {
	return [
		track(11, Y, pose.frames.map((frame, i) => [frame, LID_Y - (pose.lifts[i] ?? 0)])),
		track(11, ROTATION, pose.frames.map((frame, i) => [frame, pose.tilts[i] ?? 0])),
		track(12, Y, pose.frames.map((frame, i) => [frame, LOCK_Y - (pose.lifts[i] ?? 0)])),
	];
}
...
function squash(keys: readonly Key[]): Track[] {
	return [
		...
		track(10, SCALE_X, keys.map(([frame, value, ease]) => [frame, 1 + (1 - value) * .8, ease])),
	];
}
...
const idleChest = animation("Chest idle", 31, IDLE_FRAMES, true, [
	track(10, ROTATION, [[0, 0], [8, 0], [12, -.045], [16, .04], [20, -.03], [24, .018], [28, 0], [IDLE_FRAMES, 0]]),
	...squash([[0, 1], [28, 1], [32, .93], [37, 1.04], [42, 1], [89, 1, "in"], [93, .88, "out"], [98, 1.05], [104, .98], [110, 1], [IDLE_FRAMES, 1]]),
	...lidTracks({
	...
	track(12, ROTATION, [[0, 0], [35, 0], [40, .22], [46, -.14], [52, .07], [58, 0], [93, 0], [96, -.12], [100, .06], [104, 0], [IDLE_FRAMES, 0]]),
...
const revealChest = animation("Chest reveal", 32, REVEAL_FRAMES, false, [
	track(10, ROTATION, [[0, 0], [2, .05], [4, -.05], [6, .03], [8, 0], [REVEAL_FRAMES, 0]]),
	track(10, Y, [[0, 0], [4, 6, "out"], [9, -26, "in"], [13, 0], [REVEAL_FRAMES, 0]]),
	...squash([[0, 1], [4, .86], [9, 1.1], [13, .92], [17, 1.03], [21, 1], [62, 1, "in"], [66, .88, "out"], [71, 1.05], [76, 1], [REVEAL_FRAMES, 1]]),
	...lidTracks({
	...
	track(12, ROTATION, [[0, 0], [10, .3], [16, -.2], [22, .1], [28, 0], [66, 0], [69, -.12], [73, .06], [77, 0], [REVEAL_FRAMES, 0]]),
...
function visibility(shown: number | null, duration: number): Track[] {
	return variants.map((_, variant) =>
		track(rootId(variant), OPACITY, [[0, variant === shown ? 1 : 0, "hold"], [duration, variant === shown ? 1 : 0, "hold"]])
	);
}

const THING_ANIMATION = 100;
const noThing = animation("No thing", THING_ANIMATION - 1, 1, false, visibility(null, 1));
const thingAnimations = variants.flatMap((variant, index) => [
	animation(`Thing ${index} idle`, THING_ANIMATION + index * 2, IDLE_FRAMES, true, [
		...visibility(index, IDLE_FRAMES),
		...place(variant.action, IDLE_ACTION_START, IDLE_FRAMES),
	]),
	animation(`Thing ${index} reveal`, THING_ANIMATION + index * 2 + 1, REVEAL_FRAMES, false, [
...
const thingLayer = `<StateMachineLayer name="Thing"><AnyState x="0" y="-150">${
	...
	variants.map((_, variant) =>
		...
		}" reset="true" x="450" y="${variant * 120}"><StateTransition stateToId="0:${THING_NONE}">${
