...
pub struct RetireTemplate<'account> {
	...
	/// Instruction arguments encoded and sent as CPI data for `retire_template`.
	pub ix: RetireTemplateIx,
}

/// Instruction arguments for the `retire_template` CPI call.
#[derive(Clone, Copy, Debug, Default)]
pub struct RetireTemplateIx;

impl RetireTemplateIx {
	...
	pub fn to_bytes(&self) -> Result<[u8; 2], ProgramError> {
		let mut data = [0u8; 2];
		data[..2].copy_from_slice(&RETIRE_TEMPLATE_DISCRIMINATOR);

		Ok(data)
	}
}

impl<'account> RetireTemplate<'account> {
...
const RETIRE_TEMPLATE_DISCRIMINATOR: [u8; 2] = [21, 0];
