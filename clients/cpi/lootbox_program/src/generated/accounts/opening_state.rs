...
impl OpeningState {
	...
	pub fn matches(data: &[u8]) -> bool {
		data.len() >= 2 && data[..2] == OPENING_STATE_DISCRIMINATOR
	}
	...
	pub fn parse(data: &[u8]) -> Option<OpeningState> {
		...
		cursor += 32;
		let recipient = Address::new_from_array(data.get(cursor..cursor + 32)?.try_into().ok()?);
		cursor += 32;
