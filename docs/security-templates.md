...
| Creator edits a jackpot after market launch      | Before lock, changes are append-only and versioned. Lock proves there is no staged tail, permanently blocks additions, and leaves bundle indices, quantities, assets, amounts, reveal time, box mint, and metadata immutable.                                                           |
...
Market lock and retirement are both one-way. Market lock revokes mint authority and blocks appends while preserving transfer and later opening; retirement preserves those holder rights while stopping the creator lifecycle. Outstanding standard box tokens do not expire. A pre-lock token burned outside the program prevents lock because actual supply no longer matches recorded issuance. After a valid lock, active recovery remains unavailable until live mint supply and pending openings are zero; only still-undrawn inventory is recoverable.
...
- Surfpool executes real program/token transactions for staged funding, activation, cancellation/reclaim, exact issuance, mint-authority revocation, pre-reveal rejection, transfer, six FIFO openings, SOL/token/NFT delivery, redirect rejection, duplicate-claim rejection, retirement, recovery, and receipt closure.
