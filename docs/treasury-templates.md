...
| Token-2022 token    | Only the explicitly allowlisted Metadata Pointer and Token Metadata extensions are accepted, with no freeze authority; fee, hook, delegate, pause, and other behavior-changing extensions fail closed. The one exception is an issuer-controlled tokenized stock (PreStocks, Backed xStocks): its permanent delegate must be an allowlisted issuer key, and it may then carry a freeze authority, permanent delegate, initialized default account state, scaled UI amount, pause config (unpaused at funding), confidential-transfer config, a transfer hook with no program, and a transfer fee. Funding grosses up the fee so escrow holds exactly the recorded amount; winners receive that amount minus the issuer's fee. The issuer keeps freeze, pause, seizure, fee, and corporate-action powers the program cannot prevent; see [security notes](security-templates.md#issuer-controlled-tokenized-stocks). |
| Mint-on-claim badge | An empty zero-decimal mint irrevocably transfers mint authority to the bundle. The bundle's released count caps claims arithmetically, and final delivery revokes authority.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| Compressed NFT      | Bubblegum transfer adapter bound to the canonical asset ID, tree, leaf index, hashes, nonce, and at most 16 Merkle proof accounts. Deeper trees require canopy.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| PrizePool           | One slot holds up to 4,096 on-chain-checked immutable Bubblegum V1 assets from one tree. The pool PDA owns every deposited leaf, the manifest commits the sealed ordered semantic inventory, and committed opening entropy selects one unassigned item.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Exclusive NFT       | A slot attaches to a published, protocol-level layered collection while its window is open and commits to its frozen layer hash. Claims mint one immutable Bubblegum V2 leaf into the collection's PDA-controlled Core collection, with traits drawn from the opening's committed entropy. The attachment escrows Bubblegum's mint fee per copy; see [Exclusive Lootbox NFTs](exclusive-nfts.md).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |

Collection and compressed transfers require fresh account resolution at funding, claim, and reclaim time. The typed SDK prevents silently routing those assets through the generic token path. See [PrizePool reference](prize-pools.md) for the custody, entropy, mutation, and recovery rules.
...
| Stage      | `addBundle`, `fundSolPrize`, `fundTokenPrize`, `fundQuoteSolPrize`, `fundQuoteTokenPrize`, `fundMintPrize`, `fundMetadataNftPrize`, `fundCoreAssetPrize`, `fundCompressedNftPrize`, `createPrizePool`, `preparePrizePoolItem`, `depositPrizePoolItem`, `cancelPrizePoolItem`, `sealPrizePool`, `attachExclusiveNft`, `activateBundle`, `cancelBundle` |
| Issue/open | `mintTemplateBoxes`, `requestTemplateOpen`, `fulfillTemplateOpen`, `allocateTemplateOpen`, `allocatePrizePoolOpen`, `forfeitTemplateOpen`                                                                                                                                                                                                             |
| Deliver    | `claimSolPrize`, `claimTokenPrize`, `claimMintPrize`, `claimMetadataNftPrize`, `claimCoreAssetPrize`, `claimCompressedNftPrize`, `claimPrizePoolItem`, `claimExclusiveNft`                                                                                                                                                                            |
| Recover    | `reclaimSolPrize`, `reclaimTokenPrize`, `reclaimMintPrize`, `reclaimMetadataNftPrize`, `reclaimCoreAssetPrize`, `reclaimCompressedNftPrize`, `reclaimPrizePoolItem`, `closePrizePool`, `reclaimExclusiveFees`, `closeTemplateOpening`, `closeServiceVault`                                                                                            |
| Collection | `createExclusiveCollection`, `setExclusiveLayer`, `appendExclusiveTree`, `publishExclusiveCollection` (protocol admin)                                                                                                                                                                                                                                |
...
Replace the placeholder with a valid address before running the example. Rust exposes `TemplatePlan::new(&bundles)` with `PrizeAsset::{Sol, QuoteSol, ClassicToken, Token2022, QuoteToken, MintBadge, LegacyNft, MetadataNft, CoreAsset, CompressedNft, PrizePool, ExclusiveNft}`. Dart exposes `TemplatePlan`, `PrizeBundle`, and matching `PrizeAsset` constructors. All planners validate the 1,024-bundle limit, one-to-four asset limit, `u64` collateral math, `u32` ticket limit, unique-asset ownership, and exact PrizePool quantity.

`LootboxClient` provides resumable `createTemplate` and `appendBundles`, `publishTemplate`, `cancelFundingBundle`, exact `lockTreasury`, transfer/open/fulfill/allocate/claim orchestration, timeout forfeiture, receipt closure, and read APIs. PrizePool funding refreshes proofs before each tree write and between metadata preparation and transfer. Prize delivery keeps each asset's setup and claim atomic, partitions mixed bundles into transaction-size/account-bounded batches, and refetches the claim mask after every confirmed batch or competing-relayer conflict. Market helpers validate lock readiness, compute explicit remaining-inventory EV, quote integer-only constant-product trades, and export a checked Raydium CPMM deployment manifest. Pool creation itself stays in a production wallet/network adapter. The client uses processed commitment only for the local single-node sandbox; production applications must choose appropriate finality, transaction simulation, wallet, and oracle transport policies.
