...
pub struct SealTemplate<'account> {
	...
	/// Instruction arguments encoded and sent as CPI data for `seal_template`.
	pub ix: SealTemplateIx,
}

/// Instruction arguments for the `seal_template` CPI call.
#[derive(Clone, Copy, Debug, Default)]
pub struct SealTemplateIx;
...
impl SealTemplateIx {
	...
	pub fn to_bytes(&self) -> Result<[u8; 2], ProgramError> {
		let mut data = [0u8; 2];
		data[..2].copy_from_slice(&SEAL_TEMPLATE_DISCRIMINATOR);

		Ok(data)
	}
}

impl<'account> SealTemplate<'account> {
...
const SEAL_TEMPLATE_DISCRIMINATOR: [u8; 2] = [14, 0];
