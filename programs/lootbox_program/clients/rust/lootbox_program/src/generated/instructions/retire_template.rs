...
impl RetireTemplateInstructionData {
	pub fn new(configure: impl FnOnce(&mut RetireTemplateInstructionWireZc)) -> Result<Self, solana_program_error::ProgramError> {
		let mut bytes = vec![0u8; core::mem::size_of::<RetireTemplateInstructionWireZc>()];
		<RetireTemplateInstructionWire as pina::PinaPodFixed>::initialize(&mut bytes, |data| {
			configure(data);
			data.discriminator = RETIRE_TEMPLATE_DISCRIMINATOR;
			data.migration_version = RETIRE_TEMPLATE_MIGRATION_VERSION;
			Ok(())
		})
