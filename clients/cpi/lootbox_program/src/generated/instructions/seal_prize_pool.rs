...
pub struct SealPrizePool<'account> {
	...
	/// Instruction arguments encoded and sent as CPI data for `seal_prize_pool`.
	pub ix: SealPrizePoolIx,
}

/// Instruction arguments for the `seal_prize_pool` CPI call.
#[derive(Clone, Copy, Debug, Default)]
pub struct SealPrizePoolIx;

impl SealPrizePoolIx {
	...
	pub fn to_bytes(&self) -> Result<[u8; 2], ProgramError> {
		let mut data = [0u8; 2];
		data[..2].copy_from_slice(&SEAL_PRIZE_POOL_DISCRIMINATOR);

		Ok(data)
	}
}

impl<'account> SealPrizePool<'account> {
...
const SEAL_PRIZE_POOL_DISCRIMINATOR: [u8; 2] = [46, 0];
