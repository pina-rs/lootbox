...
pub struct FundQuoteSolPrize<'account> {
	...
	/// Instruction arguments encoded and sent as CPI data for `fund_quote_sol_prize`.
	pub ix: FundQuoteSolPrizeIx,
}

/// Instruction arguments for the `fund_quote_sol_prize` CPI call.
#[derive(Clone, Copy, Debug)]
...
impl FundQuoteSolPrizeIx {
	...
	pub fn to_bytes(&self) -> Result<[u8; 10], ProgramError> {
		let mut data = [0u8; 10];
		data[..2].copy_from_slice(&FUND_QUOTE_SOL_PRIZE_DISCRIMINATOR);
...
impl<'account> FundQuoteSolPrize<'account> {
...
const FUND_QUOTE_SOL_PRIZE_DISCRIMINATOR: [u8; 2] = [39, 0];
