...
pub struct CloseServiceVaultInstructionData {
	bytes: Vec<u8>,
}
...
impl CloseServiceVaultInstructionData {
	pub fn new(configure: impl FnOnce(&mut CloseServiceVaultInstructionWireZc)) -> Result<Self, solana_program_error::ProgramError> {
		let mut bytes = vec![0u8; <CloseServiceVaultInstructionWire as pina::ZeroPodFixed>::SIZE];
		{
			let data = <CloseServiceVaultInstructionWire as pina::ZeroPodFixed>::from_bytes_mut(&mut bytes)
				.map_err(|_| solana_program_error::ProgramError::InvalidInstructionData)?;
			configure(data);
			data.discriminator = CLOSE_SERVICE_VAULT_DISCRIMINATOR;
		}
		<CloseServiceVaultInstructionWire as pina::ZeroPodFixed>::validate(&bytes)
			.map_err(|_| solana_program_error::ProgramError::InvalidInstructionData)?;
		Ok(Self { bytes })
	}
}
...
#[doc(hidden)]
#[derive(pina::ZeroPod)]
pub struct CloseServiceVaultInstructionWire {
